...
uint8_t Humi_One_Order_Buf[96];
uint8_t * P_Humi_One_Order_Buf = 0;
extern OS_SEM     APP_UART_TX_SEM[UART_NUMBER];//串口2 用于串口2的互斥访问
extern OS_MUTEX   UART_TX_BUF_MUTEX[UART_NUMBER];//串口2 的发送缓冲区的互斥访问
extern uint8_t get_screen_timer;
void  Humi_SendChar(uchar t)
{
	 *(P_Humi_One_Order_Buf++) = t; 
}
#if(CRC16_ENABLE)
...
void BEGIN_CMD()
{
  P_Humi_One_Order_Buf = Humi_One_Order_Buf;
	TX_8(0xEE);
...
void END_CMD()
{
  ...
  TX_16(crc16);                         //发送CRC16	
	TX_32(0xFFFCFFFF)
	length = P_Humi_One_Order_Buf-Humi_One_Order_Buf;
  OSMutexPend (&UART_TX_BUF_MUTEX[1],100 ,OS_OPT_PEND_BLOCKING, &ts,&err);
	//Feed_WatchDog_Flag |= 0X04; //置位任务运行标志
  if(err == OS_ERR_NONE )
	{
		CPU_CRITICAL_ENTER();
		index = uart_trans_w_index[1];
		if(uart_trans_w_index[1] > uart_trans_r_index[1])
			size = USART2_TX_BUFF_SIZE-uart_trans_w_index[1]+uart_trans_r_index[1]-1;
		else if(uart_trans_w_index[1] < uart_trans_r_index[1])
			size = uart_trans_r_index[1] - uart_trans_w_index[1]-1;
		else 
			size = USART2_TX_BUFF_SIZE - 1;
		CPU_CRITICAL_EXIT();
		if(size >= length)
		{
			for(i=0; i<length;i++)
			{
				UART_TX_BUFFER.UART2_BUFFER[index] = Humi_One_Order_Buf[i]; 
				index = (index+1)%USART2_TX_BUFF_SIZE;
			}
			
			CPU_CRITICAL_ENTER();
			uart_trans_w_index[1] = index;
			CPU_CRITICAL_EXIT();
			OSMutexPost(&(UART_TX_BUF_MUTEX[1]),OS_OPT_POST_NO_SCHED,&err);
			OSSemPost (&APP_UART_TX_SEM[1],OS_OPT_POST_1,&err);
		}
		else            
		{
			OSMutexPost(&(UART_TX_BUF_MUTEX[1]),OS_OPT_POST_NO_SCHED,&err);
			//OSSemPost (&APP_UART_TX_SEM[1],OS_OPT_POST_1,&err);//防止发送信号量丢失，只要收到要发送的数据，就发送信号量。
		}
	}
}
...
#define SEND_DATA(P) Humi_SendChar(P)          //发送一个字节
void BEGIN_CMD(void)
{
  P_Humi_One_Order_Buf = Humi_One_Order_Buf;
	*(P_Humi_One_Order_Buf++) = 0xEE;
}
void END_CMD(void)
{
  OS_ERR   err;
...
void SetRTC(struct rtc_time * tm)
{
    u8 data[7],i=0;
	  BEGIN_CMD();
    TX_8(0x81);
    data[i++] = tm->tm_sec%10|((tm->tm_sec/10)<<4);
	  data[i++] = tm->tm_min%10|((tm->tm_min/10)<<4);
    data[i++] = tm->tm_hour%10|((tm->tm_hour/10)<<4);
	  data[i++] = tm->tm_mday%10|((tm->tm_mday/10)<<4);
	  data[i++] = tm->tm_wday%10|((tm->tm_wday/10)<<4);	
    data[i++] = tm->tm_mon%10|((tm->tm_mon/10)<<4);
	  data[i++] = (tm->tm_year-2000)%10|(((tm->tm_year-2000)/10)<<4);	
	  for(i=0; i<7 ;i++)  
			TX_8(data[i]);	
    END_CMD();
}
