...
> 版本: v3.0 | 更新: 2026-07-29 | 状态: 设计阶段完成



**环境监控系统** - 面向 10,000 台设备的远程环境监测与调控系统。

设备端 6 大功能：

| # | 功能 | 说明 |
|---|------|------|
| 1 | 环境信息检测 | 实时采集环境参数（温度等），为系统调控提供数据依据 |
| 2 | 环境调控 | 根据采集数据驱动外设（电磁阀等），自动调控环境参数（控制逻辑待定） |
| 3 | 人机交互 | 串口屏(RS232)展示 + 按键/LED/蜂鸣器，支持本地查看与参数操作 |
| 4 | 本地数据保存 | SD 卡存储环境数据/运行日志/系统参数，断网断电不丢失，可回溯查询 |
| 5 | 数据上传 | 4G 模块将环境数据/设备状态远程上传至后台服务器 |
| 6 | 固件升级 | 网页端远程固件下载、在线升级（OTA） |

系统架构：

```
设备端 (STM32F103VE + uC/OS-III)
  ├─ 环境检测 -> 本地存储 -> 4G 上传 -> 云端
  ├─ 环境调控 (10路电磁阀, 根据检测数据自动控制)
  ├─ 人机交互 (串口屏RS232 + 3按键 + 11LED + 蜂鸣器)
  └─ OTA 远程固件升级
...
| # | 功能 | 描述 | 涉及硬件 |
|---|------|------|---------|
| 1 | 环境信息检测 | 实时采集环境参数，为系统调控提供数据依据 | 8 路 DS18B20 温度传感器 |
| 2 | 环境调控 | 根据采集数据驱动外设，自动调控环境参数（控制逻辑待定） | 10 路电磁阀（三极管驱动继电器） |
| 3 | 人机交互 | 实体按键/LED/蜂鸣器实现本地交互（参数操作/状态指示/报警） | 3 按键 + 11 LED + 蜂鸣器 |
| 4 | 本地数据保存 | SD 卡存储环境数据/运行日志/系统参数，断网断电不丢失，可本地回溯查询 | SD 卡 (SPI2 + FatFs) |
| 5 | 数据上传 | 4G 模块将本地环境数据/设备状态远程上传至后台服务器 | 4G 模块 (UART2, MQTT/HTTP) |
| 6 | 固件升级 | 网页端远程固件下载、在线升级 | 4G 模块 + Bootloader + W25Q64 |
...
| 引脚 | 设备名 | 功能 | 模式 |
|------|--------|------|------|
| PA0 | LED_RS485_1_TX | 冷机485发送指示灯 | LED推挽输出 |
| PA1 | LED_RS485_1_RX | 冷机485接收指示灯 | LED推挽输出 |
| PA2 | LED_RS485_2_TX | 传感器485发送指示灯 | LED推挽输出 |
| PA3 | LED_RS485_2_RX | 传感器485接收指示灯 | LED推挽输出 |
| PA4 | FLASH_CS | W25Q64 SPI1片选 | 推挽输出 |
| PA5 | FLASH_SCK | SPI1时钟 | 复用 |
| PA6 | FLASH_MISO | SPI1主入从出 | 复用 |
| PA7 | FLASH_MOSI | SPI1主出从入 | 复用 |
| PA9 | DBG_TX | USART1_TX 调试输出 | 复用 |
| PA10 | DBG_RX | USART1_RX 调试输入 | 复用 |
| PA13 | SWDIO | SWD数据 | SWD调试 |
| PA14 | SWCLK | SWD时钟 | SWD调试 |
| PA15 | BUZZER | 蜂鸣器控制 | 三极管推挽输出 |

**PORTB**

| 引脚 | 设备名 | 功能 | 模式 |
|------|--------|------|------|
| PB0 | T7 | 温度传感器7 | 独立单线,4.2K上拉,ESD |
| PB1 | T8 | 温度传感器8 | 独立单线,4.2K上拉,ESD |
| PB5 | V6 | 电磁阀6(继电器6) | 三极管推挽输出 |
| PB6 | V7 | 电磁阀7(继电器7) | 三极管推挽输出 |
| PB7 | V8 | 电磁阀8(继电器8) | 三极管推挽输出 |
| PB8 | V9 | 电磁阀9(继电器9) | 三极管推挽输出 |
| PB9 | V10 | 电磁阀10(继电器10) | 三极管推挽输出 |
| PB10 | KEY3 | 按键3 | 浮空输入,外部上拉,RC消抖,ESD |
| PB11 | SD_CS | SD卡 SPI2片选 | 推挽输出 |
| PB12 | HMI_PWR | 屏幕电源控制 | 三极管推挽输出 |
| PB13 | SD_SCK | SPI2时钟 | 复用 |
| PB14 | SD_MISO | SPI2主入从出 | 复用 |
| PB15 | SD_MOSI | SPI2主出从入 | 复用 |

**PORTC**

| 引脚 | 设备名 | 功能 | 模式 |
|------|--------|------|------|
| PC2 | RS485_2_DE | 传感器485 DE/RE方向控制 | 推挽输出 |
| PC3 | RS485_1_DE | 冷机485 DE/RE方向控制 | 推挽输出 |
| PC4 | T5 | 温度传感器5 | 独立单线,4.2K上拉,ESD |
| PC5 | T6 | 温度传感器6 | 独立单线,4.2K上拉,ESD |
| PC6 | 4G_FACTORY | 4G模块恢复出厂 | 推挽输出 |
| PC7 | 4G_RST | 4G模块硬件复位 | 推挽输出 |
| PC8 | 4G_PWR | 4G模块电源使能 | 三极管推挽输出 |
| PC10 | 4G_TX | UART4_TX -> 4G模块 | 复用 |
| PC11 | 4G_RX | UART4_RX <- 4G模块 | 复用 |
| PC12 | RS485_1_TX | UART5_TX -> 冷机485 | 复用 |
| PC14 | LSE_IN | 32.768kHz晶振输入 | RTC |
| PC15 | LSE_OUT | 32.768kHz晶振输出 | RTC |

**PORTD**

| 引脚 | 设备名 | 功能 | 模式 |
|------|--------|------|------|
| PD2 | RS485_1_RX | UART5_RX <- 冷机485 | 复用 |
| PD3 | V1 | 电磁阀1(继电器1) | 三极管推挽输出 |
| PD4 | V2 | 电磁阀2(继电器2) | 三极管推挽输出 |
| PD5 | RS485_2_TX | USART2_TX -> 传感器485 (remap) | 复用 |
| PD6 | RS485_2_RX | USART2_RX <- 传感器485 (remap) | 复用 |
| PD7 | V3 | 电磁阀3(继电器3) | 三极管推挽输出 |
| PD8 | HMI_TX | USART3_TX -> 串口屏 (full remap) | 复用 |
| PD9 | HMI_RX | USART3_RX <- 串口屏 (full remap) | 复用 |
| PD12 | LED_4G_TX | 4G发送指示灯 | LED推挽输出 |
| PD13 | LED_4G_RX | 4G接收指示灯 | LED推挽输出 |

**PORTE**

| 引脚 | 设备名 | 功能 | 模式 |
|------|--------|------|------|
| PE0 | V4 | 电磁阀4(继电器4) | 三极管推挽输出 |
| PE1 | V5 | 电磁阀5(继电器5) | 三极管推挽输出 |
| PE4 | LED_TASK | 系统任务运行灯 | LED推挽输出 |
| PE5 | LED_RUN | 系统运行总灯 | LED推挽输出 |
| PE6 | LED_ALARM | 故障报警指示灯 | LED推挽输出 |
| PE8 | T1 | 温度传感器1 | 独立单线,4.2K上拉,ESD |
| PE9 | T2 | 温度传感器2 | 独立单线,4.2K上拉,ESD |
| PE10 | T3 | 温度传感器3 | 独立单线,4.2K上拉,ESD |
| PE11 | T4 | 温度传感器4 | 独立单线,4.2K上拉,ESD |
| PE12 | LED_USER1 | 自定义指示灯1 | LED推挽输出 |
| PE13 | LED_USER2 | 自定义指示灯2 | LED推挽输出 |
| PE14 | KEY1 | 按键1 | 浮空输入,外部上拉,RC消抖,ESD |
| PE15 | KEY2 | 按键2 | 浮空输入,外部上拉,RC消抖,ESD |

#### UART 重映射配置

| UART | 重映射 | 引脚 | 用途 |
|------|--------|------|------|
| USART1 | 无 | PA9/PA10 | 调试 |
| USART2 | 部分重映射 | PD5/PD6 | 传感器RS485 |
| USART3 | 完全重映射 | PD8/PD9 | RS232/串口屏 |
| UART4 | 无 | PC10/PC11 | 4G模块 |
| UART5 | 无 | PC12/PD2 | 冷机RS485 |

#### 设备命名规则

| 前缀 | 设备类型 | 数量 | 示例 |
|------|---------|------|------|
| T | 温度传感器 | 8 | T1(PE8) ~ T8(PB1) |
| V | 电磁阀 | 10 | V1(PD3) ~ V10(PB9) |
| LED_ | 指示灯 | 11 | LED_TASK(PE4), LED_ALARM(PE6) |
| KEY | 按键 | 3 | KEY1(PE14), KEY2(PE15), KEY3(PB10) |
| BUZZER | 蜂鸣器 | 1 | PA15 |
| HMI_ | 串口屏 | 3 | HMI_TX(PD8), HMI_RX(PD9), HMI_PWR(PB12) |
| 4G_ | 4G模块 | 5 | 4G_TX(PC10), 4G_RST(PC7) |
| RS485_1_ | 冷机485 | 3 | RS485_1_TX(PC12), RS485_1_DE(PC3) |
| RS485_2_ | 传感器485 | 3 | RS485_2_TX(PD5), RS485_2_DE(PC2) |
| FLASH_ | W25Q64 | 4 | FLASH_CS(PA4) |
| SD_ | SD卡 | 4 | SD_CS(PB11) |
