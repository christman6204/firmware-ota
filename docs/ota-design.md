...
| `file_size` | INT | 字节数 |
| `iv` | BINARY(16) | AES-CTR 初始向量（每次升级随机；仅服务端记录/审计，不下发设备） |
| `hmac` | BINARY(32) | HMAC-SHA256（覆盖 IV + 密文；仅服务端记录/审计，不下发设备） |
| `release_notes` | TEXT | 发布说明 |
| `status` | ENUM | draft/released/archived |
| `created_at` | DATETIME | |

> 说明（方案 A）：`iv`/`hmac` 随密文 blob 一同存 OSS（blob = `IV+密文+HMAC`），设备端从 blob 固定偏移读取，不依赖 MySQL 这两列。此处两列仅作服务端审计/重算校验用。
...
POST   /api/v1/firmwares/{id}/release # 发布
...
POST   /api/v1/device/heartbeat       # 心跳上报（版本/状态）
GET    /api/v1/device/ota/check       # 主动检查更新（MQTT 丢失兜底）
GET    /api/v1/ota/download/{token}   # 下载 bin（token 一次性，防盗链，支持 Range）
...
| 0x10 | MCU→ESP | 数据上报 | `dev_id[4B] + JSON` | §20 数据采集 |
| 0x11 | ESP→MCU | 数据上报 ACK | `result` | 0=ok/1=fail |
...
3. 判断触发：窗口内超阈值点数达到应有采样数（整段持续超阈值） → 写入 alerts 表 + 推送 MQTT 通知
...
### 18.1 短 key 映射表（STM32 ↔ TDengine 列名）

STM32 上报用短 key 节省 UART 带宽；taosX 订阅写入 TDengine 或 FastAPI 查询时按此表还原为标准列名。

| 短 key | TDengine 列 | 含义 | 单位 |
|---|---|---|---|
| `ts` | `ts` | 时间戳 | Unix s |
| `t` | `temp` | 温度 | °C |
| `h` | `hum` | 湿度 | % |
...
| `e` | `energy` | 累计电量 | kWh |
| `f` | `freq` | 电网频率 | Hz |
| `d1`~`d4` | `di1`~`di4` | 数字输入 | 0/1 |
| `r` | `rssi` | WiFi 信号 | dBm |
| `up` | `uptime` | 运行时间 | s |
| `x` | `payload` | 扩展字段（JSON 字符串，未建列的测点塞这里） | - |

> 新增正式列时，STM32 固件与 taosX 映射配置需同步更新此表。
