...
| 3 | 人机交互 | 串口屏可视化展示 + 实体按键/LED/蜂鸣器，支持本地查看与参数操作 |
...
| # | 功能 | 描述 | 涉及硬件 |
|---|------|------|---------|
| 1 | 环境信息检测 | 实时采集环境参数，为系统调控提供数据依据 | 10 路 DS18B20 温度传感器 |
| 2 | 环境调控 | 根据采集数据驱动外设，自动调控环境参数（控制逻辑待定） | 10 路电磁阀（三极管驱动继电器） |
| 3 | 人机交互 | 串口屏实时展示环境数据/设备状态/系统参数；实体按键/LED/蜂鸣器辅助交互 | 串口屏 + 4 按键 + 8 LED + 蜂鸣器 |
