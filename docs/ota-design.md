...
| 调试串口 | 1 | UART1 | 调试 + CLI 配置 |

| 4G 模块 | 1 | UART2 | 云端通信 (MQTT/HTTP)，厂家固件，APN 已预配 |

| 串口屏 | 1 | UART3 | HMI 可视化展示 |

| RS485 | 1 | UART4 + DE/RE | 外部设备通信 |

| RS232 | 1 | UART5 | 外部设备通信 |

| DS18B20 温度传感器 | 10 路 | 10 GPIO (独立 1-Wire) | 环境温度采集 |

| 电磁阀 | 10 路 | 10 GPIO (三极管驱动继电器) | 环境调控执行 |

| LED | 8 路 | 8 GPIO | 状态指示 |

| 按键 | 4 个 | 4 GPIO (带消抖) | 本地参数操作 |

| 蜂鸣器 | 1 路 | 1 GPIO | 报警提示 |

| SD 卡 | 1 | SPI2 + FatFs | 本地数据存储 (90 天) |

| RTC | 1 | 内置 (LSE + VBAT) | 实时时钟 |

| 片外 Flash | 1 | SPI1 (W25Q64) | OTA 固件中转 |
