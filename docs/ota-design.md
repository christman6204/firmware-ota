...
| 项目 | 说明 |
|---|---|
| 主控 MCU | STM32F103VE（裸机/RTOS，片内 flash 512KB，App 区 256KB，含 RTC + VBAT 后备） |
...
|---|---|---|---|---|
| 508KB | `0x0807F000` | 页 `0x0807F000` | 加密标记 `0x00001234`（uint32） | Bootloader 首次上电检查 |
| 510KB | `0x0807F800` | 页 `0x0807F800` | 加密 ID（16B，HMAC-SHA256 前 16 字节） | 与 UID 绑定的设备指纹 |

> 两处在不同 2KB 页，擦除标记页不会影响加密 ID 页。

**流程**：

```
设备首次上电:
  1. Bootloader 读 0x0807F000 → 检查加密标记
  2. if 标记 == 0x00001234 (出厂烧录的初始值):
     a. 读 UID (0x1FFFF7E8, 12B)
     b. 加密ID = HMAC-SHA256(主密钥, UID) 取前 16B
     c. 写加密ID 到 0x0807F800
     d. 擦除 0x0807F000 所在页 (整页变 0xFF)
...
| 固件加密性能 | STM32F103VE 软件 AES-256 ~50-80 KB/s，~256KB 加解密 3-5s，可接受 |
