...
| EMQX | MQTT 路由、设备认证；规则引擎桥接数据到 TDengine | 新增 data topic 路由 |
| TDengine | 时序数据：遥测 + 事件 + 告警 + 自动降采样聚合 | **新增组件** |
...
```sql
CREATE DATABASE iot_data KEEP 365 DAYS 10 BLOCKS 4;
...
CREATE STABLE telemetry (
  ...
  payload  NCHAR(512)    -- 扩展字段 (JSON)，常用字段后续可 ALTER STABLE 提升为正式列
) TAGS (
...
| 层 | 格式 | 说明 |
|---|---|---|
| STM32 → ESP-07S | `{"ts":1721712000,"t":25.3,"h":68.2,"v":3.31}` | 简短 key 节省 UART 带宽 |
| ESP-07S → MQTT | 同上，透传 | 不做转换 |
| FastAPI → 前端 | `{"ts":"2026-07-23T10:00:00","temperature":25.3,"humidity":68.2,"voltage":3.31}` | 完整 key，前端友好 |
...
```
data/{dev_id}/telemetry    → 遥测 JSON，EMQX 规则引擎桥接到 TDengine
...
- **清理**：`CREATE DATABASE` 已设 `KEEP 365 DAYS`，自动删过期数据
